// Portofolio
// PT Bauer Petojo
import bauer1 from "./assets/portofolio/bauer-petojo/img-1.jpeg";
import bauer2 from "./assets/portofolio/bauer-petojo/img-2.jpeg";
import bauer3 from "./assets/portofolio/bauer-petojo/img-3.jpeg";

// Hambalang
import hambalang1 from "./assets/portofolio/hambalang/img-1.jpeg";
import hambalang2 from "./assets/portofolio/hambalang/img-2.jpeg";
import hambalang3 from "./assets/portofolio/hambalang/img-3.jpeg";
import hambalang4 from "./assets/portofolio/hambalang/img-4.jpeg";

// Grand Kowloon
import kowloon1 from "./assets/portofolio/grand-kowloon/img-1.jpeg";
import kowloon2 from "./assets/portofolio/grand-kowloon/img-2.jpeg";
import kowloon3 from "./assets/portofolio/grand-kowloon/img-3.jpeg";

// CIBIS NINE
import cibis1 from "./assets/portofolio/cibis-nine/img-1.jpeg";
import cibis2 from "./assets/portofolio/cibis-nine/img-2.jpeg";

// Peruri
import peruri1 from "./assets/portofolio/peruri/img-1.jpeg";
import peruri2 from "./assets/portofolio/peruri/img-2.jpeg";
import peruri3 from "./assets/portofolio/peruri/img-3.jpeg";

// Krida Bakti
import kridaBakti1 from "./assets/portofolio/krida-bakti/img-1.jpeg";
import kridaBakti2 from "./assets/portofolio/krida-bakti/img-2.jpeg";
import kridaBakti3 from "./assets/portofolio/krida-bakti/img-3.jpeg";
import kridaBakti4 from "./assets/portofolio/krida-bakti/img-4.jpeg";

// Asrama Haji
import asramaHaji1 from "./assets/portofolio/asrama-haji/img-1.jpeg";
import asramaHaji2 from "./assets/portofolio/asrama-haji/img-2.jpeg";
import asramaHaji3 from "./assets/portofolio/asrama-haji/img-3.jpeg";

// CFX Tower
import cfxTower1 from "./assets/portofolio/cfx-tower/img-1.jpeg";
import cfxTower2 from "./assets/portofolio/cfx-tower/img-2.jpeg";
import cfxTower3 from "./assets/portofolio/cfx-tower/img-3.jpeg";
import cfxTower4 from "./assets/portofolio/cfx-tower/img-4.jpeg";

// Golf Island PIK
import golfIsland1 from "./assets/portofolio/golf-island-pik/img-1.jpeg";
import golfIsland2 from "./assets/portofolio/golf-island-pik/img-2.jpeg";

// ICE BSD
import iceBSD1 from "./assets/portofolio/ice-bsd/img-1.jpeg";

// Kantor Walikota
import kantorWalkot1 from "./assets/portofolio/kantor-walkot/img-1.jpeg";
import kantorWalkot2 from "./assets/portofolio/kantor-walkot/img-2.jpeg";
import kantorWalkot3 from "./assets/portofolio/kantor-walkot/img-3.jpeg";

// Kementrian Keuangan
import kemenkeu1 from "./assets/portofolio/kemenkeu/img-1.jpeg";
import kemenkeu2 from "./assets/portofolio/kemenkeu/img-2.jpeg";
import kemenkeu3 from "./assets/portofolio/kemenkeu/img-3.jpeg";

// Keraton Cirebon
import keratonCirebon1 from "./assets/portofolio/keraton-cirebon/img-1.jpeg";
import keratonCirebon2 from "./assets/portofolio/keraton-cirebon/img-2.jpeg";
import keratonCirebon3 from "./assets/portofolio/keraton-cirebon/img-3.jpeg";

// Hotel Santika Mega
import santikaMega1 from "./assets/portofolio/santika-mega/img-1.jpeg";
import santikaMega2 from "./assets/portofolio/santika-mega/img-2.jpeg";
import santikaMega3 from "./assets/portofolio/santika-mega/img-3.jpeg";

// Sheraton Grand
import sheratonGrand1 from "./assets/portofolio/sheraton-grand/img-1.jpeg";

// PT Aifast Indonesia
import aifastIndo1 from "./assets/portofolio/aifast-indonesia/img-1.jpeg";
import aifastIndo2 from "./assets/portofolio/aifast-indonesia/img-2.jpeg";
import aifastIndo3 from "./assets/portofolio/aifast-indonesia/img-3.jpeg";

// Gedung DPR RI
import gedungDPRRI1 from "./assets/portofolio/dpr-ri/img-1.jpeg";
import gedungDPRRI2 from "./assets/portofolio/dpr-ri/img-2.jpeg";
import gedungDPRRI3 from "./assets/portofolio/dpr-ri/img-3.jpeg";

// Elnusa TB Simatupang
import elnusaTBSimatupang1 from "./assets/portofolio/elnusa-tb-simatupang/img-1.jpeg";
import elnusaTBSimatupang2 from "./assets/portofolio/elnusa-tb-simatupang/img-2.jpeg";
import elnusaTBSimatupang3 from "./assets/portofolio/elnusa-tb-simatupang/img-3.jpeg";

// Gedung Gnarli
import gedungGnarli1 from "./assets/portofolio/gedung-gnarli/img-1.jpeg";
import gedungGnarli2 from "./assets/portofolio/gedung-gnarli/img-2.jpeg";

// Golf Island PIK 2
import golfIsland2img1 from "./assets/portofolio/golf-island-pik-2/img-1.jpeg";
import golfIsland2img2 from "./assets/portofolio/golf-island-pik-2/img-2.jpeg";
import golfIsland2img3 from "./assets/portofolio/golf-island-pik-2/img-3.jpeg";

// Kementrian PUPR
import kementrianPUPR1 from "./assets/portofolio/kementrian-pupr/img-1.jpeg";
import kementrianPUPR2 from "./assets/portofolio/kementrian-pupr/img-2.jpeg";
import kementrianPUPR3 from "./assets/portofolio/kementrian-pupr/img-3.jpeg";

// Permata Gandaria
import permataGandaria1 from "./assets/portofolio/permata-gandaria/img-1.jpeg";

// PIK 2
import PIKimg1 from "./assets/portofolio/pik-2/img-1.jpeg";
import PIKimg2 from "./assets/portofolio/pik-2/img-2.jpeg";
import PIKimg3 from "./assets/portofolio/pik-2/img-3.jpeg";

// Plaza Mandiri
import plazaMandiri1 from "./assets/portofolio/plaza-mandiri/img-1.jpeg";
import plazaMandiri2 from "./assets/portofolio/plaza-mandiri/img-2.jpeg";

// Sarana Multi
import saranaMulti1 from "./assets/portofolio/sarana-multi/img-1.jpeg";
import saranaMulti2 from "./assets/portofolio/sarana-multi/img-2.jpeg";
import saranaMulti3 from "./assets/portofolio/sarana-multi/img-3.jpeg";

// TNI Cilangkap
import TNICilangkap1 from "./assets/portofolio/tni-cilangkap/img-1.jpeg";
import TNICilangkap2 from "./assets/portofolio/tni-cilangkap/img-2.jpeg";
import TNICilangkap3 from "./assets/portofolio/tni-cilangkap/img-3.jpeg";

// Toyota Astra
import toyotaAstra1 from "./assets/portofolio/toyota-astra/img-1.jpeg";
import toyotaAstra2 from "./assets/portofolio/toyota-astra/img-2.jpeg";
import toyotaAstra3 from "./assets/portofolio/toyota-astra/img-3.jpeg";

export const allPortofolio = [
  {
    id: "kantor-walkot",
    img: kantorWalkot1,
    title: "Acara di Kantor Walikota",
    desc: "Jakarta Barat tgl 19 September 2024, produk Tenda Serut Merah Putih",
    dokumentasi: [kantorWalkot1, kantorWalkot2, kantorWalkot3],
  },
  {
    id: "krida-bakti",
    img: kridaBakti1,
    title: "Event Gedung Krida Bakti",
    desc: "tgl 19 September 2024, produk Kursi Cover Strech Putih, Meja Bulat D160 Cover Putih, Sofa Single Hitam, Meja VIP Hitam",
    dokumentasi: [kridaBakti1, kridaBakti2, kridaBakti3, kridaBakti4],
  },
  {
    id: "asrama-haji-pondok-gede",
    img: asramaHaji3,
    title: "Asrama Haji Pondok Gede",
    desc: "tgl 19 September 2024, produk Kursi Cover Strech Putih",
    dokumentasi: [asramaHaji1, asramaHaji2, asramaHaji3],
  },
  {
    id: "cibis-nine",
    img: cibis2,
    title: "Event CIBIS NINE Building",
    desc: "Jl. TB Simatupang Cilandak tgl 16 September 2024, produk Meja IBM Cover Strech Hitam",
    dokumentasi: [cibis1, cibis2],
  },
  {
    id: "gudang-peruri",
    img: peruri3,
    title: "Event Gudang Peruri Karawang",
    desc: "tgl 17 September 2024, produk Meja Kursi Taman, Sofa VIP Single Putih, Meja VIP Kaca Putih",
    dokumentasi: [peruri1, peruri2, peruri3],
  },
  {
    id: "ice-bsd",
    img: iceBSD1,
    title: "Event ICE BSD Lobby Nusantara",
    desc: "tgl 16 September 2024, produk Sofa Cream Single, Sofa Cream Double, Meja Retro Bulat Putih, Meja Barstool Kaca, Kursi Cover Strech Putih",
    dokumentasi: [iceBSD1],
  },
  {
    id: "keraton-cirebon",
    img: keratonCirebon1,
    title: "Keraton Cirebon",
    desc: "Jawa Barat tgl 23 September 2024, produk Kursi Crossback Pita Putih, Meja Taman Xtra 180 Cm Runner Putih",
    dokumentasi: [keratonCirebon1, keratonCirebon2, keratonCirebon3],
  },
  {
    id: "kemenkeu",
    img: kemenkeu3,
    title: "Kementrian Keuangan",
    desc: "Jakarta Pusat tgl 23 September 2024, produk Kursi VIP Kayu, Sofa Single Oval Putih",
    dokumentasi: [kemenkeu1, kemenkeu2, kemenkeu3],
  },
  {
    id: "cfx-tower",
    img: cfxTower1,
    title: "Gedung CFX Tower",
    desc: "Jl Gatot Subroto tgl 24 September 2024, produk Meja Kursi Taman 120",
    dokumentasi: [cfxTower1, cfxTower2, cfxTower3, cfxTower4],
  },
  {
    id: "santika-mega",
    img: santikaMega1,
    title: "Event Hotel Santika Mega",
    desc: "Bekasi tgl 25 September 2024, produk Kursi Cover Strech Hitam",
    dokumentasi: [santikaMega1, santikaMega2, santikaMega3],
  },
  {
    id: "golf-island-1",
    img: golfIsland1,
    title: "Event GOlF Island PIK",
    desc: "tgl 26 September 2024, produk Meja Kursi Taman 180, Kursi Barstool Putih",
    dokumentasi: [golfIsland1, golfIsland2],
  },
  {
    id: "sheraton-grand",
    img: sheratonGrand1,
    title: "Hotel Sheraton Grand",
    desc: "Gandaria City tgl 26 September 2024, produk Kursi Futura Cover Strech Hitam",
    dokumentasi: [sheratonGrand1],
  },
  {
    id: "grand-kowloon",
    img: kowloon1,
    title: "Grand Kowloon Resto",
    desc: "Pluit Jakarta Utara tgl 27 September 2024, produk Meja Kotak atasan Putih Skerting Maroon Runner Gold, Peralatan Catering",
    dokumentasi: [kowloon1, kowloon2, kowloon3],
  },
  {
    id: "hambalang",
    img: hambalang1,
    title: "Hambalang Bogor",
    desc: "tgl 26 September 2024, produk Meja Bulat D180 Tebar Putih Topping Putih Runner Maroon, Sofa VIP Kayu Oval, Podium Kayu",
    dokumentasi: [hambalang1, hambalang2, hambalang3, hambalang4],
  },
  {
    id: "bauer-petojo",
    img: bauer3,
    title: "PT Bauer Petojo",
    desc: "Gambir Jakarta Pusat tgl 25 September 2024, produk Tenda Plafon Biru Putih, Flooring+karpet abu-abu, Meja atasan Putih Skerting Putih, Kursi Cover Strech Putih, Sirine, Mistyfan",
    dokumentasi: [bauer1, bauer2, bauer3],
  },
  {
    id: "permata-gandaria",
    img: permataGandaria1,
    title: "Apartement Permata Gandaria",
    desc: "tgl 27 September 2024, produk Dinding Tirai Warna Cream, Lampu Sorot",
    dokumentasi: [permataGandaria1],
  },
  {
    id: "toyota-astra",
    img: toyotaAstra2,
    title: "Toyota Astra Motor",
    desc: "tgl 27 September 2024, produk Bean Bag",
    dokumentasi: [toyotaAstra1, toyotaAstra2, toyotaAstra3],
  },
  {
    id: "tni-cilangkap",
    img: TNICilangkap3,
    title: "Mabes TNI AD Cilangkap",
    desc: "tgl 28 September 2024, produk Kursi dan Meja VIP Kayu",
    dokumentasi: [TNICilangkap1, TNICilangkap2, TNICilangkap3],
  },
  {
    id: "community-park-PIK-2",
    img: PIKimg1,
    title: "Community Park PIK 2",
    desc: "tgl 28 September 2024, produk Meja Bulat D160  dan Meja Kotak atasan Putih Skerting Putih, Kursi Cover Strech Putih, Misty Cool",
    dokumentasi: [PIKimg1, PIKimg2, PIKimg3],
  },
  {
    id: "golf-island-pik-2",
    img: golfIsland2img2,
    title: "Ruko Golf Island PIK 2",
    desc: "tgl 29 September 2024, produk Tenda Dekor Juntai Putih Gold, Kursi Cover Strech Putih Pita Gold, Meja Kotak Cover tebar Putih, Mistycool",
    dokumentasi: [golfIsland2img1, golfIsland2img2, golfIsland2img3],
  },
  {
    id: "kementrian-pupr",
    img: kementrianPUPR1,
    title: "Kementrian PUPR",
    desc: "tgl 30 September 2024, produk Meja Bulat D160, Meja IMB Atasan Putih Skerting Putih Topping Hijau Tua, Kursi Cover Strech Putih Pita Hijau",
    dokumentasi: [kementrianPUPR1, kementrianPUPR2, kementrianPUPR3],
  },
  {
    id: "aifast-indonesia",
    img: aifastIndo1,
    title: "PT Aifast Indonesia",
    desc: "tgl 30 September 2024, produk Meja IMB Skerting Putih, Kursi Crossback + bantalan",
    dokumentasi: [aifastIndo1, aifastIndo2, aifastIndo3],
  },
  {
    id: "sarana-multi-infrastruktur",
    img: saranaMulti3,
    title: "PT Sarana Multi Infrastruktur",
    desc: "Sahid Sudirman Center tgl 30 September 2024, produk Kursi Cover Strech Putih",
    dokumentasi: [saranaMulti1, saranaMulti2, saranaMulti3],
  },
  {
    id: "elnusa-tb-simatupang",
    img: elnusaTBSimatupang3,
    title: "Elnusa TB Simatupang",
    desc: "tgl 30 September 2024, produk Tenda Plafon Merah Putih, Tenda Sarnaville",
    dokumentasi: [
      elnusaTBSimatupang1,
      elnusaTBSimatupang2,
      elnusaTBSimatupang3,
    ],
  },
  {
    id: "gedung-dpr-ri",
    img: gedungDPRRI1,
    title: "Gedung DPR RI",
    desc: "tgl 30 September 2024, produk Meja Bulat D160 Tebar Putih, Tenda Sarnaville 5X5m",
    dokumentasi: [gedungDPRRI1, gedungDPRRI2, gedungDPRRI3],
  },
  {
    id: "plaza-mandiri",
    img: plazaMandiri1,
    title: "Plaza Mandiri",
    desc: "Jl. Gatot Subroto Jakarta Selatan tgl 1 Oktober 2024, produk Kursi Cover Strech Hitam",
    dokumentasi: [plazaMandiri1, plazaMandiri2],
  },
  {
    id: "gedung-generali",
    img: gedungGnarli1,
    title: "Gedung Generali",
    desc: "tgl 1 Oktober 2024, produk Kursi Cover Strech Putih",
    dokumentasi: [gedungGnarli1, gedungGnarli2],
  },
];

